// Retrieve Values
document.getElementById("map").innerHTML = localStorage.map;
...
document.getElementById("redCounter").innerHTML = localStorage.redCoins;
document.getElementById("blueCounter").innerHTML = localStorage.redBoardCoins;

document.getElementById("redCoinCounter").innerHTML = localStorage.blueCoins;
document.getElementById("blueCoinCounter").innerHTML = localStorage.blueBoardCoins;

squareToCoin = localStorage.squareToCoin;
// End
...
function colorSquares(totalCol, col, start, end) {
  for (i = (start * totalCol); i < (end * totalCol); i+=totalCol) {
    ...
    document.getElementsByClassName("tileCol")[col + i].id = "border";
  }
}
...
function countCoins() {
  ...
  for (i = 0; i < rows * cols; i++){
    if(document.getElementsByClassName("tileCol")[i].id == "red") {
      if(document.getElementsByClassName("tileCol")[i].hasChildNodes()) {
        redCoinTiles += 1;
      }
    }
    else if(document.getElementsByClassName("tileCol")[i].id == "blue") {
      if(document.getElementsByClassName("tileCol")[i].hasChildNodes()) {
        blueCoinTiles += 1;
      }
    }
...
tiles.addEventListener('click', function onClick(event) {
  ...
  if(event.target.id != "border") {
    if(key == 'white' && event.target.className == "tileCol") {
      ...
      event.target.id = ""; // White is equal to no name
    }
...
function save() {
  ...
  //Save the map
  mapSave = map.innerHTML;
  localStorage.map = mapSave;
  ...
  //Save the number of possible coins and coins on boards.
  localStorage.redCoins = document.getElementById("redCounter").innerHTML;
  localStorage.redBoardCoins = document.getElementById("blueCounter").innerHTML;

  localStorage.blueCoins = document.getElementById("redCoinCounter").innerHTML;
  localStorage.blueBoardCoins = document.getElementById("blueCoinCounter").innerHTML;

  //Save sqaureToCoin ratio
  if(rows >= cols){
    greater = rows;
  }
  else if (cols > rows) {
  ...
  localStorage.squareToCoin = Math.sqrt(greater);
}